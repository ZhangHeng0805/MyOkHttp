...
import com.zhangheng.myapplication.adapter.ImgText1Adapter;
...
//                    strings=LocalFileTool.audioType;
                    LocalFileTool.readFile(strings, this, new LocalFileTool.IReadCallBack() {
                        ...
                        public void callBack(List<String> localPath) {
                            ...
                            if (strings.equals(LocalFileTool.imageType)) {
                                List<Map<String, String>> data = new ArrayList<>();
                                for (String path : localPath) {
                                    Log.e("图片地址:", path);
                                    Map<String, String> map = new HashMap<>();
                                    map.put("img", path);
                                    map.put("text", path.replace(LocalFileTool.BasePath, "内部存储"));
                                    data.add(map);
                                }
                                adapter=new ImgText1Adapter(Main5Activity.this,data);
                                Toast.makeText(Main5Activity.this,"一共找到"+data.size()+"张图片",Toast.LENGTH_SHORT).show();

                            } else {
                                adapter = new ArrayAdapter<String>(
                                        Main5Activity.this, R.layout.item_list_text, localpath);
                                Toast.makeText(Main5Activity.this,"一共找到"+localPath.size()+"个文件",Toast.LENGTH_SHORT).show();
                            }
